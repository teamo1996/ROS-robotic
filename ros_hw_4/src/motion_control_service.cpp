...
bool turtle_control(ros_hw_4::turtle_control::Request &req,
                    ros_hw_4::turtle_control::Response &res){
    ROS_INFO("%s call service",req.turtlename.data());
    ...
    for(std::map<std::string,geometry_msgs::Twist>::iterator i = m_controls.begin();i != m_controls.end();i++){
            ...
            ROS_INFO("control_msg:%s",i->first.data());
